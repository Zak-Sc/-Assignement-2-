...
parser.add_argument('--model_saved', type=str, default='')
...
def generate_samples(model,data):
    ...
    x,_=next(ptb_iterator(data,model.batch_size,model.seq_len))
    inputs=torch.from_numpy(x.astype(np.int64)).transpose(0,1).contiguous().to(device)
    model.zero_grad()
    hidden=model.init_hidden().to(device)
    hidden=repackage_hidden(hidden)
    samples=model.generate(inputs,hidden,model.seq_len).cpu().numpy()
...
if args.model_saved:
    model_stat=torch.load(args.model_saved)
